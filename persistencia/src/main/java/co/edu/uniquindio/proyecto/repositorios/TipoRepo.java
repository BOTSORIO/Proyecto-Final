...
public interface TipoRepo extends JpaRepository<Tipo,Integer> {
    ...
    @Query("select new co.edu.uniquindio.proyecto.dto.TipoMayorCalificacionDTO(t,avg(c.calificacion)) from Tipo t join t.lugares l join l.comentarios c where l.estado=true and l.ciudad.id =:idCiudad group by l.id order by avg(c.calificacion) desc")
